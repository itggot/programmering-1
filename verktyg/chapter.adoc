...
Till exempel kan man addera dividera två tal med varandra, men det går inte att dividera ett ord.
...
...
----
<1> `a_negative_number` tilldelas _minus_ 5 (heltal kan vara negative).
...
...
----
<1> Det kan se ut som strängen innehåller ett tal, men den innehåller i själva verket 4 tecken (tecknet ett, tecknet tre, tecknet tre, tecknet sju).
...
<2> Längden på strängen är 1. Alltså är sista (och första) index 0. Det finns inget (`nil`) på index 1.
